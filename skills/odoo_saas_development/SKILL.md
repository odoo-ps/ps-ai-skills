...
├── models/       *.xml  — ir.model + ir.model.fields records (named after the model, e.g. `product_template.xml`)
...
- All **custom models** must be prefixed with `x_` and use **underscores only** (no dots): `x_my_model`
...
<record id="model_estate_property" model="ir.model">
    ...
    <field name="model">x_estate_property</field>
</record>
```
...
<record id="server_action_estate_list" model="ir.actions.server">
    ...
    <field name="state">code</field>
    <field name="code"><![CDATA[
properties = request.env['x_estate_property'].search([])
